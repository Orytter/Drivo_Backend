...
export const getCarOptions = async (req, res) => {
  try {
    let language = setLanguage(req.query);
    ...
    const [brand, fuelType, priceRange, bodyType, transmissionType] = await Promise.all([
      ...
      // Select all brands
      sequelize.query('select data_value as value from equipment where schema_id = 128 group by data_value', {
        type: sequelize.QueryTypes.SELECT,
      }),

      // Select all fuel Type
      sequelize.query(`SELECT data_value as code ,TRIM(TRAILING "\r" FROM full_text) as value FROM schema_text WHERE schema_id=8702 and data_value!="" and data_value!="?" and language_id=${language} `, {
        type: sequelize.QueryTypes.SELECT,
      }),

      sequelize.query('SELECT MIN(CAST(data_value AS DECIMAL(10, 2))) AS min, MAX(CAST(data_value AS DECIMAL(10, 2))) AS max FROM equipment WHERE schema_id = 906', {
        type: sequelize.QueryTypes.SELECT,
      }),

      sequelize.query(`SELECT data_value as code ,TRIM(TRAILING "\r" FROM full_text) as value FROM schema_text WHERE schema_id=606 and data_value!="" and data_value!="?" and language_id=${language}`, {
        type: sequelize.QueryTypes.SELECT,
      }),

      sequelize.query(`SELECT data_value as code ,TRIM(TRAILING "\r" FROM full_text)  as value FROM schema_text WHERE schema_id=20602 and data_value!="" and data_value!="?" and language_id=${language} `, {
        type: sequelize.QueryTypes.SELECT,
      }),
...
export const getCarBrandsInfo = async (req, res) => {
  try {
    ...
    res.status(200).json({ status: true, totalBrand: total, data: mergedData });
  } catch (error) {
    console.log(error.message);
    res.status(500).json({ status: false, error });
  }
...
///8/nov
  ...
  // });
    ...
    res.status(200).json({ status: true, totalResult: totalResult, data: carModels });
...
export const getCarMinMaxPrice = async (req, res) => {
  try {
    let language = setLanguage(req.query);
    const result = await sequelize.query('SELECT MIN(CAST(data_value AS DECIMAL(10, 2))) AS min, MAX(CAST(data_value AS DECIMAL(10, 2))) AS max FROM equipment WHERE schema_id = 906', {
      type: sequelize.QueryTypes.SELECT,
    });
    ...
    res.status(200).json({
      status: true,
      message: messages[language].CAR_PRICE_RANGE,
      data: { min, max },
    });
  } catch (error) {
    console.error(error);
...
export const getCarBodyType = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      status: true,
      message: messages[language].BODY_TYPE_RETRIEVED_SUCCESS,
...
export const getCarFuelTypes = async (req, res) => {
  try {
    let language = setLanguage(req.query);
    ...
    res.status(200).json({
      status: true,
      message: messages[language].CAR_FUEL_TYPE_RETRIEVED_SUCCESS,
      data: fuelTypes,
    });
  } catch (error) {
    console.error(error);
...
export const getCarAllTrimLevels = async (req, res) => {
  try {
    ...
    const versionSchemaMap = {
      "19": "302",  // language_id : schema_id
      "24": "131"
    };
    const trimLevelSchemaMap = {
      "19": "404",  // language_id : schema_id
      "24": "402"
    };
    if (!brand || !model || !bodyType) {
      return res.status(401).json({
        status: false,
        message: messages[language].MISSING_FIELD
      });
    }
    const trimLevels = await sequelize.query(`
    SELECT DISTINCT
    e.vehicle_id,
    e.data_value AS brand,
    b.data_value AS model,
...
WHERE
    ...
    res.status(200).json({
      status: true,
      message: messages[language].CAR_TRIM_LEVEL_SUCCESS,
...
export const getCarModelInfo = async (req, res) => {
  try {
    ...
    WHERE
        ...
        AND b.data_value = :model
    `, {
      replacements: { brand: brandName, model: modelName },
      type: sequelize.QueryTypes.SELECT,
    });

    const totalCount = carModels.length;
    res.status(200).json({ status: true, totalCount: totalCount, data: carModels });
...
export const saveSelectedVersionsInCharts = async (req, res) => {
  try {
    ...
    const totalCount = await Charts.count({
      where: {
        type,
        is_deleted: false,
      },
    ...
    if (totalCount >= 10) {
      return res.status(200).json({ status: false,totalCount: totalCount, error: `Maximum 10 records can be created in Charts for type ${type}` });
    }
    ...
    res.status(200).json({ status: true, message: messages[language].VERSION_SAVED_SUCCESS });
  } catch (error) {
...
export const saveFavorite = async (req, res) => {
  try {
    ...
    if (totalCount >= 5) {
      ...
      const oldestFavorite = await Favorite.findOne({
        where: { is_deleted: false },
        order: [['createdAt', 'ASC']], // Find the oldest entry
      });

      if (oldestFavorite) {
        await oldestFavorite.update({ is_deleted: true }); // Mark it as deleted
      }
    }
    ...
    res.status(200).json({ status: true, message:  messages[language].FAVOURITE_SAVED_SUCCESS });
  } catch (error) {
    res.status(500).json({ status: false, error: error.message });
  }
};
...
export const getCarColorwithPrice = async (req, res) => {
  let language = setLanguage(req.query);
  if (!language) {
    language = '24'
  }
...
export const getPresentPageData = async (req, res) => {
  ...
  try {
    ...
    const powerTrainResult = await sequelize.query(
...
WHERE
  ...
  const cargoSpaceResult = await sequelize.query(
    `
    SELECT data_value
    FROM
      capacity
    ...
    const updateDimensionFields = (el, targetObject) => {
      ...
      //  break;
        ...
        default:
          return null;  // No matching schema_id
      }
    }
    ...
    //   break;
     ...
     case 9003:
      ...
      break;
          ...
          default:
            return null; // Default if schema_id doesn't match known cases
        }
      
        // Set combined result for other_mobile_systems
        if (
          targetObject.other_mobile_systems_1 === 'Yes' &&
          targetObject.other_mobile_systems_2 === 'Yes' &&
          targetObject.other_mobile_systems_3 === 'Yes'
        ) {
  ...
  structuredData.Dimension[0].cargo_space = cargoSpace;
    ...
    queryResult
      .filter(item => schemaIds.Performance.includes(item.schema_id))
      .map(el => {
         updatePerformanceFields(el,structuredData.Performance[0])
      })
      queryResult
      .filter(item => schemaIds.Dimension.includes(item.schema_id))
      .map(el => {
        updateDimensionFields(el,structuredData.Dimension[0])
      })
      queryResult
      .filter(item => schemaIds.SafetyAndSecurity.includes(item.schema_id))
      .map(el => {
        updateSafetyField(el,structuredData.SafetyAndSecurity[0])
      })
      queryResult
      .filter(item => schemaIds.Overview_of_Car.includes(item.schema_id))
      .map(el => {
        updateOverviewOfCar(el,structuredData.Overview_of_Car[0])
      })
      queryResult
      .filter(item => schemaIds.Comfort.includes(item.schema_id))
      .map(el => {
...
export const getOptionBuildRules = async (req, res) => {
  let language = setLanguage(req.query);
  try {
    const { vehicle_id, option_id } = req.query;
    if (!vehicle_id) {
      return res.status(400).json({
        status: false,
        message: messages[language].VEHICLE_ID_AND_Language_ID_ERROR,
    ...
    let resultSets;
    if (language === '19') {
      resultSets = await sequelize.query(`
      SELECT 
      e.option_id,
      e.vehicle_id,
      ol.id_902 AS price,
      ...
      ORDER BY 
        option_id;
      `, {
        replacements: { option_id, vehicle_id },
        type: sequelize.QueryTypes.SELECT,
      });
