...
const sequelizeDBV2 = new Sequelize('DRIVO_DBV4', 'drivodb', 'smaarst@a20299', {
  host: '18.184.209.15',
  dialect: 'mysql',
  port: 3306
});

const sequelizeDrivoTest = new Sequelize('drivotest', 'drivodb', 'smaarst@a20299', {
  host: '18.184.209.15',
  dialect: 'mysql',
  port: 3306
});
...
async function getDeactivatedDatabase() {
  const [result] = await sequelizeDrivoTest.query(
    `SELECT * FROM DatabasesTable WHERE status = 0 LIMIT 1`
  );
...
async function getLastUpdateTime(databaseId) {
  const [result] = await sequelizeDrivoTest.query(
    `SELECT * FROM DatabaseLogs WHERE database_id = ${databaseId} ORDER BY updatedAt DESC LIMIT 1`
  );
...
async function toggleDatabaseStatuses(db_id) {
  await sequelizeDrivoTest.query(
    `UPDATE DatabasesTable SET status = IF(id = '${db_id}', 1, 0)`
  );
...
const downloadPath = path.join(__dirname, '../../../../../../var/Drivo_Backend/public/assets/downloadsftptest', zipFileName);
const extractPath = path.join(__dirname, '../../../../../../var/Drivo_Backend/public/assets/downloadsftptest', zipFileName.replace('.zip', ''));
...
async function getLatestZipFile(sftp) {
  ...
  return { path: path.join(remoteDir, latestZipFile.name), modifyTime: latestZipFile.modifyTime };
}
...
function setPermissionsRecursively(directoryPath) {
  try {
    ...
    fs.chmodSync(directoryPath, 0o777); // Full access for all users
    ...
    files.forEach(file => {
      ...
      if (stats.isDirectory()) {
        ...
        fs.chmodSync(filePath, 0o777); // Read/write for all users
      }
    });
  } catch (error) {
  ...
  try {
   ...
   // Call the stored procedure to truncate tables
    ...
    const loadDataScript = [
      `LOAD DATA INFILE "${dataPath}/SSCDK_version_utf8.txt" IGNORE INTO TABLE version IGNORE 1 LINES;`,
      `LOAD DATA INFILE "${dataPath}/SSCDK_equipment_utf8.txt" IGNORE INTO TABLE equipment IGNORE 1 LINES;`,
      `LOAD DATA INFILE "${dataPath}/SSCDK_option_list_utf8.txt" IGNORE INTO TABLE option_list IGNORE 1 LINES;`,
      `LOAD DATA INFILE "${dataPath}/SSCDK_option_build_utf8.txt" IGNORE INTO TABLE option_build IGNORE 1 LINES;`,
      `LOAD DATA INFILE "${dataPath}/SSCDK_option_name_utf8.txt" IGNORE INTO TABLE option_name IGNORE 1 LINES;`,
      `LOAD DATA INFILE "${dataPath}/SSCDK_option_text_utf8.txt" IGNORE INTO TABLE option_text IGNORE 1 LINES;`,
      `LOAD DATA INFILE "${dataPath}/SSCDK_schema_info_utf8.txt" IGNORE INTO TABLE schema_info IGNORE 1 LINES;`,
      `LOAD DATA INFILE "${dataPath}/SSCDK_schema_text_utf8.txt" IGNORE INTO TABLE schema_text IGNORE 1 LINES;`,
      `LOAD DATA INFILE "${dataPath}/SSCDK_standard_text_utf8.txt" IGNORE INTO TABLE standard_text IGNORE 1 LINES;`
    ];
...
async function downloadAndExtractAndUpdate(dbChoice, isManualUpdate = false, manualExtractPath = null) {
  ...
  try {
    ...
    if (isManualUpdate) {
      ...
      const [result] = await sequelizeDrivoTest.query(
        `SELECT * FROM DatabasesTable WHERE Database_Name = '${dbChoice}' LIMIT 1`
      );
      ...
      await sequelizeDrivoTest.query(
        `INSERT INTO DatabaseLogs (database_id, path, updatedAt) VALUES (${deactivatedDb.id}, '${manualExtractPath}', NOW())`
      );
    ...
    await sftp.connect(sftpConfig);
    const { path: zipFilePath, modifyTime } = await getLatestZipFile(sftp);
    ...
    // Log the update in DatabaseLogs
    await sequelizeDrivoTest.query(
      `INSERT INTO DatabaseLogs (database_id, path, updatedAt) VALUES (${deactivatedDb.id}, '${zipFilePath}', NOW())`
    );
...
export { downloadAndExtractAndUpdate };