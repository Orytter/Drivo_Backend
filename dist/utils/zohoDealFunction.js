...
const {
  frontendUrl
} = _config.default.get("frontendUrl");
console.log(frontendUrl);
const createDeal = async (latestUserID, contactId, Date_Created, zohoCredential, Make, Model, Version, Vehicle_Type, Base_Price, Base_Price_Delivered, Options_Price, Total_Price_Delivered, Configuration, vehicleId, stage, req) => {
  // Fetch the latest deal record
  const DealformRecord = await _clientDealForm.default.findOne({
  ...
  await _clientDealForm.default.create(dealDataLocal);

  // Send the deal data to Zoho CRM
  const dealResponse = await _axios.default.post('https://www.zohoapis.eu/crm/v2/Deals', {
    data: [dealDataLocal]
  }, {
    headers: {
  ...
  if (dealResponse.data && dealResponse.data.data && dealResponse.data.data[0].status === 'success') {
    ...
    // Handle file upload
    const filePath = _path.default.join(__dirname, `../../${req.file.path}`);
    console.log("filePath1212", filePath);
    const fileContent = _fs.default.createReadStream(filePath);
    ...
    const pdfResult = await _axios.default.post(config.url, data, {
